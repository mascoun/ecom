 ...
 */
package tn.iw.ecom.cockpits.setup;
...
import tn.iw.ecom.cockpits.constants.EcomCockpitsConstants;
 ...
 */
@SystemSetup(extension = EcomCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(EcomCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(EcomCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						EcomCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/ecomcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
